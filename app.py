...
def upload():
    ...
    if request.method == "POST":
        ...
        conn.execute(
            "UPDATE users SET rewards = rewards + 1 WHERE id = ?",
            (session['user_id'],)
        )
        conn.commit()

        # Fetch updated rewards for display
        user = conn.execute(
            "SELECT rewards FROM users WHERE id = ?",
            (session['user_id'],)
        ).fetchone()
        conn.close()
...
def profile():
    ...
    if request.method == "POST":
        ...
        conn = get_db_connection()
        conn.execute(
            "UPDATE users SET username = ?, password = ?, profile_image = ? WHERE id = ?",
            (new_username, new_password, filename, session["user_id"])
        )
        conn.commit()
        conn.close()
